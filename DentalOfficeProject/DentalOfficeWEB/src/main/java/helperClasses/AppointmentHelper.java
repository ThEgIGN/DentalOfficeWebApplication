...
public class AppointmentHelper {
	...
	public Object[] removeAnAppointmentFromListOfLists(List<List<Appointment>> listOfAppointmentsForDates,
		...
		// JPA and I should just make new clean object and dump values in it
		...
		if (deletionAppointment.getLength() == (MINUTES * 2)) {
			...
			// current position, so when we add first time to the list to the same position,
			// second one will be pushed to the right
			LocalTime secondAppointmentTimeEndLocal = timeEnd.toLocalTime();
			LocalTime secondAppointmentTimeStartLocal = secondAppointmentTimeEndLocal.minusMinutes(MINUTES);
			Time secondAppointmentTimeStart = new Time(secondAppointmentTimeStartLocal.getHour(),
					secondAppointmentTimeStartLocal.getMinute(), 0);
			Time secondAppointmentTimeEnd = new Time(secondAppointmentTimeEndLocal.getHour(),
					secondAppointmentTimeEndLocal.getMinute(), 0);
			appointmentsList.set(innerCounter,
					new Appointment(date, MINUTES, secondAppointmentTimeEnd, secondAppointmentTimeStart, dentist));
			cleanAppointment.setTimeEnd(secondAppointmentTimeStart);
			appointmentsList.add(innerCounter, cleanAppointment);
		} else {
		...
		boolean deleteCheck = appointmentManager.removeAppointment(deletionAppointment);
		if (deleteCheck) {
			listOfAppointmentsForDates.set(outerCounter, appointmentsList);
			return new Object[] { true, listOfAppointmentsForDates, deletionAppointment };
		} else {
			return new Object[] { false, listOfAppointmentsForDates, deletionAppointment };
		}
	}
