...
public class PatientsExistingAppointmentsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (checkDates) {
			...
			if (deleteCheck) {
				Appointment emailAppointment = appointmentsList.remove(appointmentListIndex);
				...
				// If before using this servlet, patient looked at appointments for specific
				// date and then decided to cancel appointment for that date, update list in
				// session so when patient goes back to look at appointments, they can see that
				// time space available and not still reserved
				Date dateAppointment = null;
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				String dateAppointmentString = (String) request.getSession().getAttribute("patientAppointmentsDate");
				...
				if (dateAppointment != null && dateAppointment.equals(emailAppointment.getDate())) {
					appointmentHelper = new AppointmentHelper();
					List<Appointment> appointments = appointmentHelper
							.createListWithAllAppointmentsForDate(dateAppointment);
					request.getSession().setAttribute("patientAvailableAppointments", appointments);
				}
